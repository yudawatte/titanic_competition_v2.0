...
# Evaluate Model
evaluate_model(voting_clf, 'Soft Voting Classifier', X_train, y_train, 0.3)
...
#clf_performance(voting_clf, "Voting Classifier")
...
# Saving the model
print("\nSavning Model")
#save_model(voting_clf, "vc_soft_model.sav")
save_model(voting_clf, "vc_hard_model.sav")
...
#save_data(base_submission, sett.RESULT_DATA_PATH, "vc_soft_results.csv", index=False, header=True)
save_data(base_submission, sett.RESULT_DATA_PATH, "vc_hard_results.csv", index=False, header=True)

"""
Accuracy with train set - soft voting:  77.6
Accuracy with train set - hard voting:  83.15
